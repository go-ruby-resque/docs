...
`go-ruby-resque/resque` reimplements Ruby's `resque` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-resque/resque` directly, with no Ruby runtime;
...
Because `resque` is pure Go with cgo disabled, it also compiles to **WebAssembly**
